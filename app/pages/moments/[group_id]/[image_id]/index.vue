				...
				<pane :size="paneRight" class="pl-3 overflow-hidden border-l mb-36 md:mb-auto">
					<div class="md:h-[82.5vh] overflow-scroll">
						...
						<div class="" v-if="content.has_interactions">
							<div class="sticky top-0 z-40 pb-2 bg-white">
								<div class="flex items-center justify-between w-full gap-2 mt-4 mb-1">
									<div>
									...
									<button class="flex items-center justify-center gap-2 p-2 px-4 text-sm text-white bg-[#756145] border border-[#756145] rounded-xl">
										...
										<UtilsCounter :count="comments_count" />
									</button>
								</div>
							...
							<div v-if="content.has_interactions" class="flex flex-col gap-3 mt-3">
								<div v-for="comment in comments" class="flex items-start gap-2 p-2 bg-gray-100 rounded-lg">
									...
									<div class="flex flex-col w-full">
										<div class="flex items-center justify-start gap-2">
											...
											<p class="text-xs text-gray-500">{{ useDateFormat(comment.created_at, "DD-MM-YYYY - HH:mm") }}</p>
										</div>
										<p class="text-sm text-gray-700 text-balance">{{ comment.content.text }}</p>
										<div class="flex items-center justify-end pt-2 mt-2 border-t border-gray-300">
											<div class="flex items-center justify-end gap-2">
												<button @click="deleteComment(comment.id)" class="flex items-center gap-2 p-2 text-sm text-white bg-[#756145] border border-[#756145] rounded-xl">
													<Icon name="ri:close-circle-line" size="1.2rem" />
												</button>

												<button @click="editComment(comment.id)" class="flex items-center gap-2 p-2 text-sm text-white bg-[#756145] border border-[#756145] rounded-xl">
													<Icon name="ri:edit-2-line" size="1.2rem" />
												</button>

												<button @click="focusEditable" class="flex items-center gap-2 p-2 pr-3 text-sm text-white bg-[#756145] border border-[#756145] rounded-xl">
