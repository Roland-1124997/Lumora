	...
	<div>
				...
				<pane :size="paneRight" class="pl-3 overflow-hidden border-l mb-36 md:mb-auto">
					<div class="md:h-[82.5vh] overflow-scroll">
						...
						<div class="" v-if="content.has_interactions">
							<CardCommentsForm :count="comments_count" :isAnimating :onSubmit="handleSumbitComments" ref="mobileCommentForm" />
							<div class="flex flex-col gap-3 mt-3">
								<CardComments v-for="comment in comments" :key="comment.id" :content="comment" :permisions="content?.permision" :onDelete="() => {}" :onEdit="() => {}" :onReply="focusEditable" />
							</div>
						</div>
					</div>
				</pane>
		...
		<div class="md:hidden">
			<div class="w-full h-full">
				<div class="pl-3 -mt-4 border-l md:pr-3 mb-36">
					<CardImageThumbnail :loaded :content ref="thumbnail" />
					...
					<hr class="my-2 mt-4" />
					<div class="" v-if="content.has_interactions">
						<CardCommentsForm :count="comments_count" :isAnimating :onSubmit="handleSumbitComments" ref="commentForm" />
						<div class="flex flex-col gap-3 mt-3">
							<CardComments v-for="comment in comments" :key="comment.id" :content="comment" :permisions="content?.permision" :onDelete="() => {}" :onEdit="() => {}" :onReply="focusEditable" />
						</div>
					</div>
...
<script setup>
	...
	const handleSumbitComments = async (comment) => {
		await $fetch(`/api/moments/comments/${group_id}/${content.value.id}`, { method: "POST", body: { comment: comment } }).then(async (response) => {
			await makeRequest(`/api/moments/comments/${group_id}/${content.value.id}`);
			if (data.value) comments.value = data.value.data;
