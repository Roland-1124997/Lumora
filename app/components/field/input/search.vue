<template>
	<div class="w-full">
		<input id="search" :disabled="disabled || loading" v-model="term" @input="debouncedSearch" type="search" :placeholder="placeholder ? placeholder : 'Search...'" class="flex-grow w-full p-2 px-3 border bg-white border-[#756145] ring-[#756145] outline-none appearance-none rounded-xl focus:ring-1" />
	</div>
</template>
...
<script setup lang="ts">
	...
	const { makeRequest } = useRetryableFetch({ throwOnError: true });
	...
	const debouncedSearch = useDebounce(async () => {
		...
		const multiple = atributes ? "&" : "?";
		const { data, error } = await makeRequest(`${uri}${multiple}search=${term.value}`);
    ...
    watch(loading, () => {
		update(response, failure, loading.value, term.value);
	});
</script>
