export function useRetryableFetch(configuration: { maxAttempts?: number; delay?: number; throwOnError?: boolean } = {}) {
	...
	const fetchWithRetry = async <T>(url: Parameters<typeof $fetch>[0], options: Parameters<typeof $fetch>[1] & { sessions?: boolean }, attempts = 0): Promise<ApiResponse<T>> => {
		...
		return $fetch<ApiResponse<T>>(url, withHeaders)
			...
			.catch((error: ErrorResponse) => {
				if (++attempts < maxAttempts) {
					return new Promise<ApiResponse<T>>((resolve) => setTimeout(resolve, delay)).then(() => fetchWithRetry<T>(url, withHeaders, attempts));
				}
	...
	const makeRequest = async <T>(url: Parameters<typeof $fetch>[0], options: Parameters<typeof $fetch>[1] & { sessions?: boolean } = { sessions: true }) => {
